#!/bin/bash
DEB_VER="8.8" #switch to 9.5 or 8.8
docker rm xrootdBuilder-$DEB_VER &>/dev/null
docker build --tag xrootdbuilder-$DEB_VER:0.1 --tag xrootdbuilder-$DEB_VER:latest --build-arg XRD_VER="4.11.3" --build-arg DEB_VER="debian:$DEB_VER" --build-arg ADDITIONAL_VERSION_STRING="-gsi-2" .
